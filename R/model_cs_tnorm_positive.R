...
par_cs_tnorm_positive_check <- function(params,
                               indeces,
                               y, X,
                               CM = NULL,
                               CV_u,
...
t_par_cs_tnorm_positive <- function(pars){
  # pars <- sqrt(exp(pars))
...
ll_cs_tnorm_positive_contrib <- function(params,
                                indeces,
                                y, X,
                                CV_u,
                                CV_v,
...
ll_cs_tnorm_positive <- function(params,
                        indeces,
                        y, X,
                        CV_u,
                        CV_v,
  ...
  lli <-
    ll_cs_tnorm_positive_contrib(params,
                        indeces,
                        y, X,
                        CV_u,
                        CV_v,
...
if (require(compiler)) ll_cs_tnorm_positive <- cmpfun(ll_cs_tnorm_positive)
...
g_cs_tnorm_positive_analytic <- function(params,
                                indeces,
                                y, X,
                                CV_u,
                                CV_v,
...
g_cs_tnorm_positive_fd <- function(params,
                          indeces,
                          y, X,
                          CV_u,
                          CV_v,
  ...
  sapply(1:n, function(i) {
    (ll_cs_tnorm_positive(params + hh[i, ], indeces, y, X, CV_u, CV_v, CM, ineff, minmax, deb) -
       ll_cs_tnorm_positive(params - hh[i, ], indeces, y, X, CV_u, CV_v, CM, ineff, minmax, deb)) / (2 * .Machine$double.eps^(1/3))})
}
if (require(compiler)) g_cs_tnorm_positive_fd <- cmpfun(g_cs_tnorm_positive_fd)


# efficiency --------------------------------------------------------------

u_cs_tnorm_positive <- function(object, estimator) {
